...
const sourceURL = new URL("../../contracts/src/SleepRewardToken.sol", import.meta.url);
const source = await readFile(sourceURL, "utf8");
const input = {
  language: "Solidity",
  sources: { "SleepRewardToken.sol": { content: source } },
  settings: { optimizer: { enabled: true, runs: 200 }, outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const errors = (output.errors ?? []).filter((entry) => entry.severity === "error");
if (errors.length) throw new Error(errors.map((entry) => entry.formattedMessage).join("\n"));
const artifact = output.contracts["SleepRewardToken.sol"].SleepRewardToken;
...
if (!receipt.contractAddress || receipt.status !== "success") throw new Error("token deployment failed");
